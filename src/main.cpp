...
uint256 hashGenesisBlock("0x1b4cb1eb7a04b4efcabee7d743e54286f565062389db0276ea3b206fee384937");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Negotium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Negotium Signed Message:\n";
...
{
    // Negotium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 65536);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 18000;
static const int64 nTargetSpacing = 540;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Negotium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Negotium: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Negotium: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1b4cb1eb7a04b4efcabee7d743e54286f565062389db0276ea3b206fee384937");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1396593961;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086024233;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NegotiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NegotiumMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Negotium with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NegotiumMiner terminated\n");
        throw;
    }
}
